...
use crate::config::{BATCH_INBOX, BATCHER, OPTIMISM_PORTAL};
use crate::deposits::TRANSACTION_DEPOSITED_TOPIC;
use crate::DepositedTransaction;
...
async fn run_subscription(
    ...
    while let Some(msg) = read.next().await {
        let msg = match msg {
            ...
            Err(e) => {
                let _ = tx
                    .send(Err(PipelineError::WebSocket(e.to_string())))
                    .await;
                break;
            }
        };
        ...
        .and_then(|t| t.as_array())
    {
        for tx_val in txs {
            ...
            if to == Some(batch_inbox) && from == Some(batcher) {
                tracing::info!(
                    "Found batch tx in L1 block {} (type {:?})",
                    block_number,
                    tx_type
                );
                ...
                let blob_hashes = if tx_type == Some(3) {
                    tx_val
                        ...
                        .map(|arr| {
                            arr.iter()
                                .filter_map(|h| h.as_str()?.parse::<B256>().ok())
                                .collect()
                        })
                        .unwrap_or_default()
                } else {
